...
def diff_of_gauss (data, narrow_width, wide_width):
    ...
    #else:
    auss_img_small = signal.convolve2d(data, narrow_kern,mode='same',boundary='symm') #convolve DOG with image
    auss_img_big = signal.convolve2d(data, wide_kern, mode='same', boundary='symm')  # convolve DOG with image
    diff_gauss_img = auss_img_small - auss_img_big
    return diff_gauss_img

data = load_img('/Users/Ashley/Downloads/sequence/00011.tif')
